...
Handles arbitrary rotation angles with intelligent cropping and masking
"""
...
class RotationProcessor:
    ...
    def process_cuts_with_rotation(self, input_prefix='output', output_folder='rotated_cuts', 
        ...
        for i in range(cuts_count):
            ...
            if self.verbose:
                print(f"Processing {input_filename} with rotation angle: {angle:.1f}°")
            
            try:
                rotated_cropped = self.rotate_with_crop(image, angle)
                ...
                cv.imwrite(output_filename, rotated_cropped)
                processed_count += 1
                
                if self.verbose:
                    original_size = f"{image.shape[1]}x{image.shape[0]}"
                    final_size = f"{rotated_cropped.shape[1]}x{rotated_cropped.shape[0]}"
    ...
    def process_single_image(self, image_path, angle, output_path):
        """
        ...
        try:
            ...
            rotated_cropped = self.rotate_with_crop(image, angle)
            cv.imwrite(output_path, rotated_cropped)
    ...
    def batch_process_with_angle_range(self, input_files, output_folder, min_angle, max_angle, samples_per_image=5):
        """
        ...
        for file_path in input_files:
            ...
            for sample in range(samples_per_image):
                ...
                if self.process_single_image(file_path, angle, output_path):
                    total_processed += 1
...
def validate_rotation_args(rotation_angle, rotation_range):
    ...
    if rotation_angle is None and rotation_range is None:
        return False  # No rotation specified, that's okay
